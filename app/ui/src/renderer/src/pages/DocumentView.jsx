...
const DocumentView = () => {
  ...
  // Solicitamos el documento real al backend a través del comando "getDocument"
  useEffect(() => {
    const fetchDocument = () => {
      setLoading(true);
      // Se envía el comando con el id; el backend responde con "getDocumentsSuccess"
      sendCommand("getDocuments", { id: parseInt(id) });
      console.log(`Enviando comando "getDocuments" con id ${id} al backend...`);
    };

    const unsubscribeSuccess = onEvent("getDocumentsSuccess", (data) => {
      console.log("getDocumentsSuccess received in DocumentView:", data);
      if (Array.isArray(data)) {
        if (data.length > 0) {
          setDocument(data[0]);
          toast.success(`Documento cargado: ${data[0].title}`);
        } else {
          toast.error("No se encontró ningún documento");
        }
      } else if (data && typeof data === "object" && data.id && data.title) {
        setDocument(data);
        toast.success(`Documento cargado: ${data.title}`);
      } else {
        console.error("Formato de datos inválido:", data);
        toast.error("Formato de datos inválido en el documento");
      }
      setLoading(false);
    });

    const unsubscribeFailure = onEvent("getDocumentFailure", (errorData) => {
      console.error("Error al cargar el documento:", errorData);
      toast.error(errorData.error || "Error al cargar el documento");
      setLoading(false);
    });
  ...
  const handleOpenFile = () => {
    if (document.main_path) { // o document.filePath, según el nombre que uses
      window.api.openFile(document.main_path)
        .then((result) => {
          if (result) {
            // Si openPath devuelve una cadena, hubo un error
            toast.error("Error al abrir el archivo: " + result);
          }
        })
        .catch((err) => {
      ...
      toast.error("La ruta del archivo no está definida.");
    }
  };
  const handleMarkAsRelevant = () => {
    toast.success("Documento marcado como relevante");
    // Lógica para marcar el documento como relevante en el backend
  };
